...
function slideNoHist(oldElID,newElID,direction){
   var newEl = document.getElementById(newElID + '-A');
    var oldEl = document.getElementById(oldElID + '-A');
   console.log(`sliding ${oldElID} to ${direction} to make room for ${newElID} `);
//    newEl.scrollTop = 0;
   newEl.setAttribute('class', 'section-A');
   oldEl.setAttribute('class', 'section-A slide-out-' + direction);
   newEl.scrollTop = 0;
   currentElID = newElID;
}
